...
(function () {
  ...
  const CONFIG = {
    ...
    serviceQr: 'assets/images/service-qr.png'
  };
  ...
  /* ---------- 会员数据（数据驱动，?id=编号） ---------- */
  const RAW_MEMBERS = (window.RAW_FEMALE_TEST || []).concat([
    ...
    { id:18, name:'Mike',   gender:'男', img:18, age:42, city:'纽约', district:'', height:170, zodiac:'射手座', job:'企业主', income:'30万以上', marriage:'离异', education:'硕士', housing:'有(无贷款)', car:'有', interests:'旅行、商务、红酒、健身', intro:'精通英文、中文、粤语、福州话和西班牙语的多语种企业主，纽约城市大学硕士毕业。在纽约有房有车，事业稳定，年收入30万美元+。离异无子女，希望找到理想伴侣组建幸福家庭。真诚靠谱，期待与你相遇。', birth:'1984-12-15', blood_type:'', weight:0, wechat:'', uid:'00018', updated:'2026-07-29', photo:'assets/images/member-00018.jpg', expect_gender:'女', expect_age:'18-36岁', expect_height:'155-175cm', expect_education:'本科', expect_marriage:'不限', expect_income:'不限', expect_marry_time:'1年内', purpose:'寻找结婚对象', expect_other:'真诚 顾家' }
  ]);
  ...
  function initMemberDetail() {
    ...
    // 头像：优先使用详情页专用照片(detailPhoto)，其次photo，否则本地生成SVG
    box.onerror = null;
    if (m.detailPhoto) {
      box.src = m.detailPhoto;
      box.onerror = function () { this.src = m.photo || avatarURI(m.name); };
    } else if (m.photo) {
      box.src = m.photo;
